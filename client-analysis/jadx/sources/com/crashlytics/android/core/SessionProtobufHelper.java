...
class SessionProtobufHelper {
    ...
    private static int getSessionAppSize(ByteString packageName, ByteString apiKey, ByteString versionCode, ByteString versionName, ByteString installUuid, int deliveryMechanism, ByteString unityVersion) {
        ...
        int size2 = size + CodedOutputStream.computeBytesSize(2, versionCode) + CodedOutputStream.computeBytesSize(3, versionName);
        int orgSize = getSessionAppOrgSize(apiKey);
        int size3 = size2 + CodedOutputStream.computeTagSize(5) + CodedOutputStream.computeRawVarint32Size(orgSize) + orgSize + CodedOutputStream.computeBytesSize(6, installUuid);
        if (unityVersion != null) {
            size3 = size3 + CodedOutputStream.computeBytesSize(8, UNITY_PLATFORM_BYTE_STRING) + CodedOutputStream.computeBytesSize(9, unityVersion);
        }
        return size3 + CodedOutputStream.computeEnumSize(10, deliveryMechanism);
    }
    ...
    private static int getEventDeviceSize(Float batteryLevel, int batteryVelocity, boolean proximityEnabled, int orientation, long heapAllocatedSize, long diskUsed) {
        int size = batteryLevel != null ? 0 + CodedOutputStream.computeFloatSize(1, batteryLevel.floatValue()) : 0;
