package com.prime31.GPlayUtils;
...
public class Base64 {
    ...
    public static String encode(byte[] source, int off, int len, byte[] alphabet, boolean doPadding) {
        byte[] outBuff = encode(source, off, len, alphabet, Strategy.TTL_SECONDS_INFINITE);
    ...
    public static byte[] decode(byte[] source, int off, int len, byte[] decodabet) throws Exception {
        ...
        while (true) {
            ...
            if (sbiDecode >= -5) {
                ...
                throw new Exception("Bad Base64 input character at " + i + ": " + ((int) source[i + off]) + "(decimal)");
            }
        }
        if (b4Posn != 0) {
            if (b4Posn == 1) {
