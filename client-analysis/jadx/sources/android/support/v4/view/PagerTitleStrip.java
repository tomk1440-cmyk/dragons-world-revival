...
public class PagerTitleStrip extends ViewGroup implements ViewPager.Decor {
    ...
    void updateText(int currentItem, PagerAdapter adapter) {
        ...
        int childWidthSpec = View.MeasureSpec.makeMeasureSpec(maxWidth, ExploreByTouchHelper.INVALID_ID);
        int childHeight = (getHeight() - getPaddingTop()) - getPaddingBottom();
        int maxHeight = Math.max(0, childHeight);
        int childHeightSpec = View.MeasureSpec.makeMeasureSpec(maxHeight, ExploreByTouchHelper.INVALID_ID);
